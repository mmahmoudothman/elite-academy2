...
const LandingPage: React.FC = () => {
  ...
  const handleApplyClick = () => setIsRegOpen(true);
  ...
  const handleProspectusClick = () => {
    toast.success(t.landing?.prospectus_toast || 'The prospectus will be sent to your email address.');
  };
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 sm:gap-12 lg:gap-16 items-center">
                  <div className="lg:col-span-5 space-y-5 sm:space-y-8">
                    ...
                    <p className="text-base sm:text-lg text-slate-600 font-medium leading-relaxed">{t.advantage.desc}</p>

                    <div className="space-y-3 sm:space-y-4 pt-2 sm:pt-4">
                      {[
                        { title: t.advantage.item1_title, desc: t.advantage.item1_desc, icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
                        { title: t.advantage.item2_title, desc: t.advantage.item2_desc, icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129" /></svg> },
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div className="max-w-3xl mx-auto space-y-6 sm:space-y-8 lg:space-y-10">
              ...
              <div className="pt-6 sm:pt-8 lg:pt-10 flex flex-wrap justify-center gap-6 sm:gap-8 lg:gap-10">
                {[
                  { label: t.enterprise.lms, icon: <svg className="w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg> },
                  { label: t.enterprise.insights, icon: <svg className="w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg> },
                  { label: t.enterprise.skills, icon: <svg className="w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04c0 4.835 1.39 9.14 3.846 12.714a11.952 11.952 0 0013.39 0c2.456-3.574 3.846-7.88 3.846-12.714z" /></svg> },
                ].map((item, i) => (
                  <div key={i} className="flex flex-col items-center">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-slate-50 rounded-2xl sm:rounded-3xl mb-3 sm:mb-4 flex items-center justify-center border border-slate-100 shadow-sm transition-all hover:scale-110 hover:shadow-lg">{item.icon}</div>
                    <span className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-slate-900">{item.label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto innovative-gradient rounded-2xl sm:rounded-3xl lg:rounded-[4rem] p-6 sm:p-10 lg:p-16 xl:p-24 relative overflow-hidden text-center lg:text-left rtl:text-right">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center gap-8 sm:gap-10 lg:gap-16">
              ...
              <div className="lg:w-1/3 w-full flex flex-col gap-3 sm:gap-4">
                ...
                <button onClick={handleProspectusClick} className="bg-slate-900/40 backdrop-blur-md text-white border-2 border-white/20 px-6 sm:px-8 lg:px-10 py-4 sm:py-5 lg:py-6 rounded-xl sm:rounded-2xl lg:rounded-[2rem] font-black text-sm sm:text-base lg:text-xl hover:bg-white/10 transition-all active:scale-95">{t.apply_section.cta_secondary}</button>
              </div>
            </div>
          </div>
        </section>
