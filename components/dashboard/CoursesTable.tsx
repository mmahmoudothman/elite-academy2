...
const CoursesTable: React.FC<CoursesTableProps> = ({ courses, onAdd, onEdit, onDelete }) => {
  const { t } = useLanguage();
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  <th className="text-left rtl:text-right px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{t.dashboard.field_image}</th>
                  <th className="text-left rtl:text-right px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{t.dashboard.col_title}</th>
                  <th className="text-left rtl:text-right px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{t.dashboard.col_category}</th>
                  <th className="text-left rtl:text-right px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{t.dashboard.col_instructor}</th>
                  <th className="text-left rtl:text-right px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{t.dashboard.col_price}</th>
                  <th className="text-left rtl:text-right px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{t.dashboard.col_level}</th>
                  <th className="text-left rtl:text-right px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{t.dashboard.col_enrolled}</th>
                  <th className="text-left rtl:text-right px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{t.dashboard.col_rating}</th>
              ...
              <tbody>
                {courses.map((course) => (
                  <tr key={course.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-600 font-medium">{course.enrolled.toLocaleString()}</td>
                    <td className="px-6 py-4">
