  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
        <h2 className="text-2xl font-black text-slate-900">{d?.newsletter_title || 'Newsletter Subscribers'}</h2>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center">
          <p className="text-slate-400 font-medium">{d?.no_subscribers_found || 'No subscribers found'}</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  <th className="text-start px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{d?.col_email || 'Email'}</th>
                  <th className="text-start px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{d?.col_subscribed_date || 'Subscribed Date'}</th>
                  <th className="text-start px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{d?.col_source || 'Source'}</th>
                  <th className="text-start px-6 py-4 font-black text-xs uppercase tracking-widest text-slate-400">{d?.col_active || 'Active'}</th>
                ...
                {filtered.map((sub) => (
                  <tr key={sub.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        {sub.isActive !== false ? (d?.active_label || 'Active') : (d?.unsubscribed_label || 'Unsubscribed')}
                      </button>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {sub.isActive !== false && (
                          <button
                            ...
                            title={d?.unsubscribe || "Unsubscribe"}
                          >
